        ...
        with DatabaseManager(database_name) as db:
            ...
            if health_status.get("connection", False):
                try:
                    ...
                    diagnostic_results["available_tables"] = [
                    ...
                    if "processed_surveys" in diagnostic_results["available_tables"]:
                        survey_count_query = (
                            "SELECT COUNT(*) as count FROM processed_surveys"
                        )
                        survey_count = db.execute_query(survey_count_query)
                        diagnostic_results["processed_surveys_count"] = survey_count[0][
                            "count"
                        ]
                    ...
                    if "customer_orders" in diagnostic_results["available_tables"]:
                        ...
                        diagnostic_results["customer_orders_count"] = orders_count[0][
                            "count"
                        ]
                    ...
                    if "flow_execution_logs" in diagnostic_results["available_tables"]:
                        ...
                        diagnostic_results["recent_flow_executions"] = logs_count[0][
                            "count"
                        ]

                except Exception as diag_error:
                    logger.warning(
                        f"Diagnostic queries failed for '{database_name}': {diag_error}"
                    )
            ...
            comprehensive_results = {
                ...
                "overall_status": _determine_overall_health_status(
                    health_status, pool_status, migration_status
                ),
            }

            logger.info(
                f"Comprehensive health check completed for '{database_name}': {comprehensive_results['overall_status']}"
            )
...
def validate_flow_prerequisites(
    ...
    logger = get_run_logger()
    logger.info(
        f"Validating flow prerequisites for {len(required_databases)} databases"
    )
        ...
        try:
            ...
            if db_priority < min_priority:
                critical_failures.append(
                    f"Database '{db_name}' health level '{db_status}' below minimum '{minimum_health_level}'"
                )
            elif db_status == "degraded":
                warnings.append(
                    f"Database '{db_name}' is degraded but meets minimum requirements"
                )
            ...
            if required_tables and db_name in required_tables:
                ...
                available_tables = health_results.get("diagnostics", {}).get(
                    "available_tables", []
                )
                ...
                if missing_tables:
                    critical_failures.append(
                        f"Database '{db_name}' missing required tables: {missing_tables}"
                    )
                else:
                    logger.info(
                        f"All required tables present in database '{db_name}': {required_table_list}"
                    )
            ...
            if pending_migrations:
                warnings.append(
                    f"Database '{db_name}' has {len(pending_migrations)} pending migrations"
                )

        except Exception as e:
            logger.error(
                f"Prerequisite validation failed for database '{db_name}': {e}"
            )
            critical_failures.append(
                f"Failed to validate database '{db_name}': {str(e)}"
            )
    ...
    if critical_failures:
        ...
        logger.error(
            f"Flow prerequisite validation failed: {len(critical_failures)} critical issues"
        )
...
@task
def monitor_database_health_trends(
    database_names: list[str], hours_back: int = 24
) -> dict[str, Any]:
    """
    ...
    logger = get_run_logger()
    logger.info(
        f"Monitoring health trends for {len(database_names)} databases over {hours_back} hours"
    )
        ...
        try:
            with DatabaseManager(db_name) as db:
                ...
                trend_results["database_trends"][db_name] = {
                    ...
                    "success_rate": (
                        db_successful_executions / db_total_executions * 100
                    )
                    if db_total_executions > 0
                    else 0,
                ...
                logger.info(
                    f"Database '{db_name}' trends: {db_successful_executions}/{db_total_executions} successful, avg response: {db_avg_response_time:.2f}ms"
                )
    ...
    logger.info(
        f"Overall health trends: {successful_executions}/{total_executions} successful executions"
    )
    return trend_results


def _determine_overall_health_status(
    health_status: dict, pool_status: dict, migration_status: dict
) -> str:
    """
...
def health_check_integration_flow(
    ...
    if target_databases is None:
        target_databases = [
            "rpa_db"
        ]  # Only use rpa_db as SurveyHub might not be configured
    ...
    try:
        ...
        logger.info(
            f"Step 1: Performing comprehensive health checks on {len(target_databases)} databases"
        )
        ...
        if perform_trend_analysis:
            ...
            trend_results = monitor_database_health_trends(
                target_databases, hours_back=24
            )
        else:
        ...
        if not can_proceed and fail_on_prerequisites:
            ...
            logger.warning(
                "Flow prerequisites not met, but continuing due to fail_on_prerequisites=False"
            )
        else:
            logger.info(
                "All flow prerequisites validated successfully, flow can proceed"
            )
        ...
        complete_results = {
            ...
            "recommendations": _generate_health_recommendations(
                health_check_results, prerequisite_results, trend_results
            ),
        }
...
def _generate_health_recommendations(
    ...
    for db_name, health_data in health_results.items():
        ...
        if status == "critical_failure":
            recommendations.append(
                f"CRITICAL: Database '{db_name}' requires immediate attention - connection failed"
            )
        elif status == "degraded":
            response_time = health_data.get("basic_health", {}).get(
                "response_time_ms", 0
            )
            if response_time > 1000:
                recommendations.append(
                    f"WARNING: Database '{db_name}' has slow response times ({response_time}ms) - investigate performance"
                )
        ...
        if utilization > 80:
            recommendations.append(
                f"WARNING: Database '{db_name}' connection pool utilization is high ({utilization}%) - consider increasing pool size"
            )
        ...
        if len(pending_migrations) > 0:
            recommendations.append(
                f"INFO: Database '{db_name}' has {len(pending_migrations)} pending migrations - schedule maintenance window"
            )
    ...
    if trend_results:
        ...
        if success_rate < 95:
            recommendations.append(
                f"WARNING: Overall success rate is {success_rate:.1f}% - investigate recent failures"
            )

        degradation_events = overall_trends.get("health_degradation_events", 0)
        if degradation_events > 5:
            recommendations.append(
                f"WARNING: {degradation_events} health degradation events detected in last 24 hours - monitor database performance"
            )
    ...
    if critical_issues:
        recommendations.append(
            f"CRITICAL: {len(critical_issues)} prerequisite failures must be resolved before production deployment"
        )

    if not recommendations:
        recommendations.append(
            "INFO: All health checks passed - systems are operating normally"
        )
