...
def test_database_health_monitoring_flow(mock_monitoring_tasks):
    ...
    mock_monitoring_tasks["health_summary"].assert_called_once_with(["test_db1", "test_db2"])
...
def test_database_prerequisite_flow_failure(mock_monitoring_tasks):
    ...
    with disable_run_logger():
        ...
        with pytest.raises(RuntimeError, match="Database prerequisite validation failed"):
            database_prerequisite_flow(
                database_names=["test_db"],
                fail_on_validation_error=True
            )
...
def test_database_diagnostics_flow(mock_monitoring_tasks):
    ...
    mock_monitoring_tasks["health_check"].assert_called_once_with("test_db", include_retry=False)
...
def test_database_diagnostics_flow_with_issues(mock_monitoring_tasks):
    ...
    assert any("Consider increasing connection pool size" in rec for rec in recommendations)
...
def test_operational_monitoring_flow_default_databases(mock_monitoring_tasks):
    ...
    mock_monitoring_tasks["health_summary"].assert_called_once_with(["rpa_db", "SurveyHub"])
