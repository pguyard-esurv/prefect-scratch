...
def test_validate_users_with_invalid_data():
    ...
    mixed_users = [
        {"id": 1, "name": "Alice", "email": "alice@example.com", "active": True},  # Valid
        {"id": 2, "name": "", "email": "bob@example.com", "active": True},  # Invalid: empty name
        {"id": 3, "name": "Charlie", "email": "invalid-email", "active": True},  # Invalid: bad email
        {"id": 0, "name": "Diana", "email": "diana@example.com", "active": True},  # Invalid: bad ID
    ]
...
def test_generate_validation_report_logic():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        report_file = (
            output_dir / f"validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
