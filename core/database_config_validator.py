...
def validate_connection_string(connection_string: str, db_type: str) -> list[str]:
    ...
    if db_type == "postgresql":
        if not parsed.scheme or parsed.scheme not in ["postgresql", "postgres"]:
            errors.append(
                f"PostgreSQL connection string must start with 'postgresql://' or 'postgres://', got '{parsed.scheme}://'"
            )
        ...
        if parsed.query:
            ...
            if invalid_params:
                errors.append(
                    f"Unknown PostgreSQL parameters: {', '.join(invalid_params)}"
                )

    elif db_type == "sqlserver":
        if not parsed.scheme or not parsed.scheme.startswith("mssql"):
            errors.append(
                f"SQL Server connection string must start with 'mssql+pyodbc://', got '{parsed.scheme}://'"
            )
        ...
        if parsed.query:
            ...
            if "driver" not in query_params:
                errors.append(
                    "SQL Server connection string missing required 'driver' parameter"
                )
            else:
...
def validate_database_config(
    database_name: str, config_manager: Optional[ConfigManager] = None
) -> dict[str, Any]:
    """
    ...
    if not db_type:
        ...
        if db_type not in ["postgresql", "sqlserver"]:
            result["errors"].append(
                f"Invalid database type '{db_type}' for '{database_name}'. Must be 'postgresql' or 'sqlserver'"
            )
            result["valid"] = False
    ...
    try:
        ...
        if pool_size_int < 1:
            ...
            result["warnings"].append(
                f"Pool size {pool_size_int} is very high, consider reducing"
            )
    ...
    try:
        ...
        if max_overflow_int < 0:
            result["errors"].append(
                f"Max overflow must be >= 0, got {max_overflow_int}"
            )
            result["valid"] = False
        elif max_overflow_int > 100:
            result["warnings"].append(
                f"Max overflow {max_overflow_int} is very high, consider reducing"
            )
        result["config"]["max_overflow"] = max_overflow_int
    except ValueError:
        result["errors"].append(
            f"Invalid max overflow '{max_overflow}', must be an integer"
        )
        result["valid"] = False
...
def validate_all_database_configurations(
    ...
    for key, _value in os.environ.items():
        if key.startswith(f"{environment}_GLOBAL_") and key.endswith("_TYPE"):
            ...
            if len(parts) >= 4:
                db_name = "_".join(
                    parts[2:-1]
                ).lower()  # Join middle parts and lowercase
...
def test_database_connectivity(
    ...
    result = {"connected": False, "error": None, "response_time_ms": None}

    try:
        # Get connection configuration
        db_type = config_manager.get_variable(f"{database_name}_type")
        connection_string = config_manager.get_secret(
            f"{database_name}_connection_string"
        )
        ...
        engine = create_engine(
            connection_string, pool_size=1, max_overflow=0, pool_pre_ping=True
        )
...
def generate_configuration_report(
    ...
    if not validation_results:
        ...
        report_lines.append(
            f"  {config_manager.environment.upper()}_GLOBAL_{{DATABASE_NAME}}_TYPE"
        )
        report_lines.append(
            f"  {config_manager.environment.upper()}_GLOBAL_{{DATABASE_NAME}}_CONNECTION_STRING"
        )
        return "\n".join(report_lines)
    ...
    if overall_valid:
        report_lines.append(
            "✅ OVERALL STATUS: ALL DATABASES CONFIGURED AND ACCESSIBLE"
        )
    else:
        report_lines.append(
            "❌ OVERALL STATUS: CONFIGURATION OR CONNECTIVITY ISSUES FOUND"
        )
    report_lines.append("=" * 60)
